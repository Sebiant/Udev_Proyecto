...
switch ($accion){
    ...
    case 'editar':
        ...
        $sql_select = "SELECT * FROM salones WHERE id_salon='$id_salon'";  // Cambio: id_salon
    ...
    default:
        $sql = "SELECT s.id_salon, s.capacidad, s.descripcion, s.estado, i.nombres 
                FROM salones s
                JOIN instituciones i ON s.id_institucion = i.id_institucion";  // Cambio: se agregó estado
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $row['estado'] = $row['estado'] == 1 ? "activo" : "inactivo";  // Cambio: estado correcto
                $data[] = $row;
            }
        }
